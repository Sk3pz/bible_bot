...
pub async fn run(options: &[ResolvedOption<'_>], ctx: &Context, command: &CommandInteraction, guild: &GuildId) {
    let Some(ResolvedOption { value: ResolvedValue::String(option), .. }) = options.first() else {
        // error message
        command_response(ctx, command, "You must specify an option: `daily_verse`, `reading_schedule`, or `remove`").await;
        return;
    };
    ...
    match option.to_lowercase().as_str() {
        "daily_verse" => {
            ...
            command_response(ctx, command, "Channel is now registered as the daily verse channel!").await;
        }
        "reading_schedule" => {
            guild_file.set_reading_schedule_channel(channel.id.clone());
            command_response(ctx, command, "Channel is now registered as the reading schedule channel").await;
        }
        "remove" => {
            let channel_id = channel.id.get();
            guild_file.clear_channel_by_id(channel_id);
            command_response(ctx, command, "Channel has been removed from the registered channels.").await;
        }
        _ => {
            // error message
            command_response(ctx, command, "You must specify an option: `daily_verse`, `reading_schedule`, or `remove`").await;
